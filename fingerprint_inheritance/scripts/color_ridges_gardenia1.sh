...
bin/fp.out $FPFOLDER"transparency_"$FP1".tif" -sourceAFIS -boundingBox  -erosion 60 > /dev/null
bin/fp.out $FPFOLDER"transparency_"$FP2".tif" -sourceAFIS -boundingBox  -erosion 60 > /dev/null

//generazione del file comparison

COMPFILE=$FPFOLDER"operations/"$FP1"-"$FP2"Minutia.txt"
if test -f "$COMPFILE"; then
    echo "Comparison file found."
else
    COMPFILE=$FPFOLDER"comparison_results/"$FP2"-"$FP1"Minutia.txt"
    if test -f "$COMPFILE"; then
      echo "Comparison file found."
    else
...
mv "ridges_color"$FP1".json" $FPFOLDER"transparency_$FP1.tif""/inheritance/""ridges_color.json"
mv "ridges_color"$FP2".json" $FPFOLDER"transparency_$FP2.tif/inheritance/""ridges_color.json"
