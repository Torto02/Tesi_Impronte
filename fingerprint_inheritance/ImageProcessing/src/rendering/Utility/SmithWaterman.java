...
public class SmithWaterman {
    ...
    private int length1, length2; //the length of the input strings
    private double[][] score; //the score matrix, the true scores should be divided by the normalization factor
    static final double NORM_FACTOR = 1.0; //the normalization factor, to get the true score, divide the integer score used in computation by normalization factor
    //the similarity function constants, they are amplified by the normalization factor to be integers
    static final int MATCH_SCORE = 10; //10
    static final int MISMATCH_SCORE = -8; //-10
    static final int INDEL_SCORE = -9; //-10
    //constants of directions, multiple directions are stored by bits, the zero diretcion is the starting point
...
//		}
    ...
    private void printAlignments(int i, int j, String aligned1, String aligned2) {
    	...
    	if ((prevCells[i][j] & DR_ZERO) > 0) {
		    ...
		    // Note: we could check other directions for longer alignments, with the same score. we don't do it here.
		    return;
		}
    ...
    //given the bottom right corner point trace back the top left corner. at entry: i, j hold bottom right (end of aligment coords) at return: hold top left (start of alignment coords)
